/**
 * app/api/admin/daily-quests/route.ts — Sprint 4 (M4: audit logging added)
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await service.from("daily_quests").insert({
    ...
    if (error) {
      if (error.code === "23505")
        return NextResponse.json({ error: `A daily quest with id "${body.id}" already exists` }, { status: 409 });
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    await auditLog(service, user.id, "UPDATE", "daily_quest", id, before, data);
    return NextResponse.json({ quest: data });
  } catch (res) { return res as Response; }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { count } = await service.from("daily_quest_logs")
      .select("id", { count: "exact", head: true }).eq("quest_id", id);
    if ((count ?? 0) > 0)
      return NextResponse.json({ error: `Cannot delete — ${count} user(s) have completion logs for this quest. Disable it instead.` }, { status: 409 });
    ...
    await auditLog(service, user.id, "DELETE", "daily_quest", id, before, null);
