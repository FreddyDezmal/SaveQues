...
export async function POST(req: NextRequest) {
  ...
  if (error) {
    if (error.code === "23505") return NextResponse.json({ error: `A weekly quest with id "${body.id}" already exists` }, { status: 409 });
...
export async function DELETE(req: NextRequest) {
  ...
  // Guard: refuse hard delete if referenced as a step requirement in any quest chain.
