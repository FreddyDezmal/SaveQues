...
import WeeklyReflectionModal from "@/components/gamification/weeklyReflectionModal";
import { ChevronRight, Plus, PauseCircle, PlayCircle, Star } from "lucide-react";
...
interface Props {
  ...
  reflectionData: (ReflectionData & { id: string }) | null;
}
...
export default function DashboardClient({
  ...
  const [showReflection, setShowReflection] = useState(!!reflectionData);

  // Locale-aware currency formatter using user's profile settings
  ...
  // Secondary goals — everything except primary
  const secondaryGoals = activeGoals.filter((g: any) => g.id !== primaryGoal?.id);
    ...
    <>
      <div className="max-w-lg mx-auto px-4 pt-6 pb-4 bg-mesh min-h-screen">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <h2 className="font-display font-semibold text-white text-sm">Primary Goal</h2>
              </div>
              <Link href="/goals" className="text-brand-400 text-xs hover:text-brand-300">
                All goals →
              </Link>
            </div>
        ...
        <div className="grid grid-cols-3 gap-2.5 mb-4">
          <StatCard label="Total Saved" value={fc(totalSaved)} icon="💰" />
                ...
                {pauseConfirm ? (
                  <div className="px-4 py-3 rounded-xl bg-surface-elevated border border-surface-border">
                    <p className="text-xs text-white/60 mb-2.5">
                      Pause your streak for 7 days? It will not break while paused. Use this if you are travelling, busy, or just need a rest.
                    </p>
        ...
        {secondaryGoals.length > 0 && (
          <div className="mb-4">
            <div className="flex items-center justify-between mb-2">
              <h2 className="font-display font-semibold text-white text-sm">Other Goals</h2>
              <Link href="/goals/new" className="flex items-center gap-1 text-brand-400 text-xs hover:text-brand-300">
                <Plus size={13} /> New
              </Link>
            </div>
            <div className="space-y-3">
              {secondaryGoals.slice(0, userStage === "new" ? 1 : 2).map((goal: any) => (
                <GoalCard key={goal.id} goal={goal} />
              ))}
              {secondaryGoals.length > (userStage === "new" ? 1 : 2) && (
                <Link href="/goals" className="block text-center text-brand-400 text-sm py-2 hover:text-brand-300">
                  +{secondaryGoals.length - (userStage === "new" ? 1 : 2)} more →
                </Link>
              )}
            </div>
          </div>
        ...
        {userStage === "new" && (
          <div className="mb-4 card p-4 border-brand-500/20">
            ...
            <p className="text-sm text-white/60">
              Complete your daily quest and log a saving to build your streak. More features unlock as you progress.
            </p>
          </div>
        )}
      </div>
