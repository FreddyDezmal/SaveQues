...
export default function QuestChainsClient({ chains, progress, userId }: Props) {
  ...
  async function claimStep(chain: QuestChain, stepNumber: number) {
    ...
    await supabase.from("quest_chain_progress").update({
    ...
    }).eq("user_id", userId).eq("chain_id", chain.id);

    // Award XP
    const totalXP = step.xpReward + (isLastStep ? chain.completionXP : 0);
    const { data: profile } = await supabase.from("profiles").select("xp_total, quest_chains_completed").eq("id", userId).single();
    if (profile) {
      await supabase.from("profiles").update({
        xp_total: profile.xp_total + totalXP,
        quest_chains_completed: isLastStep ? (profile.quest_chains_completed ?? 0) + 1 : profile.quest_chains_completed,
      }).eq("id", userId);
    }

    await supabase.rpc("log_activity", { p_user_id: userId, p_xp: totalXP });
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {!isStarted && (
                        <span className="text-[10px] text-white/30">{chain.steps.length} steps</span>
                      )}
                      {isComplete && (
