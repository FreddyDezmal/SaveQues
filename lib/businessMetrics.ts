...
// ── Sprint 13 P2-A: Dashboard P95 check ──────────────────────────────────────

export async function checkDashboardP95(): Promise<void> {
  const supabase = createServiceClient();

  const { data, error } = await supabase.rpc("get_dashboard_p95", { p_hours: 24 });
  ...
  for (const row of (data as any[])) {
    ...
    if ((row.samples ?? 0) < THRESHOLDS.MIN_SAMPLE_SIZE) continue;

    if (row.p95_ms > 3000) {
      captureWarning("Dashboard P95 exceeds 3s threshold", {
        user_stage: row.user_stage ?? "all",
